...
@bot.tree.command(name="info", description="Показать справку по боту")
async def info_command(interaction: discord.Interaction) -> None:
    """Отправляет подробное описание возможностей бота."""

    logger.info("Получена команда info: пользователь=%s", interaction.user)

    sections = [
        "\n".join(
            [
                "**Как работает бот**",
                "Бот помогает управлять рецептами крафта: хранит составы, считает стоимость производства с учётом цен ресурсов и эффективности, ведёт отдельные затраты на создание и чертежи, а также уведомляет об обновлениях и поддерживает систему заявок на крафт.",
                "Все slash-команды доступны через меню Discord. При вводе числовых значений используйте точку или запятую как разделитель, числа будут автоматически нормализованы.",
                "Если команда предлагает автодополнение (например, названия рецептов, ресурсов или типов кораблей), начните вводить название и выберите подходящий вариант из списка.",
            ]
        ),
        "\n".join(
            [
                "**Работа с рецептами**",
                "• `/add_recipe` — добавляет или обновляет рецепт. Укажите имя, тип корабля, количество результата и приложите таблицу с компонентами (текстом или файлом). Формат таблицы: каждая строка `ресурс | количество`, значения разделяются точкой с запятой или табуляцией. Импортируйте данные прямо из игры: выберите рецепт, создайте список, откройте список сырья, скопируйте его в буфер обмена и вставьте в поле команды.",
                "• `/set_blueprint_components` — фиксирует ресурсы чертежа. Используйте тот же формат таблицы, что и для рецепта.",
                "• `/set_recipe_blueprint_cost`, `/set_blueprint_creation_cost`, `/set_recipe_creation_cost` — задают дополнительные стоимости. Значения принимаются в числовом формате без единиц измерения.",
                "• `/price` — считает стоимость цикла и единицы продукции. Можно указать свою эффективность (в процентах) или использовать глобальные/типовые значения.",
                "• `/resource_price` — показывает текущую цену ресурса.",
                "• `/audit_recipe_types` — помогает найти рецепты без указанного типа, чтобы заполнить информацию для аналитики.",
            ]
        ),
        "\n".join(
            [
                "**Настройка эффективности и панелей**",
                "• `/set_efficiency` — задаёт глобальную эффективность по умолчанию (доступно администраторам).",
                "• `/set_ship_type_efficiency`, `/delete_ship_type_efficiency`, `/ship_type_efficiencies`, `/global_efficiency` — управляют эффективностью по типам кораблей и показывают сводку.",
                "• `/set_settings_console_channel` и `/refresh_settings_console` — публикуют и обновляют консоль настроек эффективности в выбранном канале.",
                "• `/set_recipe_console_channel` и `/refresh_recipe_console` — размещают и обновляют панель для быстрого добавления рецептов.",
                "При назначении каналов убедитесь, что у бота есть права на чтение, запись и управление сообщениями.",
            ]
        ),
        "\n".join(
            [
                "**Управление ценами и обновлениями**",
                "• Группа `/graph` (`set_channel`, `add_role`, `remove_role`, `clear_roles`, `list_roles`) отвечает за систему заявок на крафт: размещение сообщения с кнопкой в канале и список ролей для уведомлений.",
                "• `/update_bot` — обновляет код с GitHub и при необходимости перезапускает сервис (требуются права администратора). После обновления бот сообщает результат и канал последней команды сохраняется для уведомления при рестарте.",
                "• Бот автоматически ведёт базу данных цен ресурсов. Чтобы данные оставались актуальными, регулярно обновляйте рецепты и стоимость компонентов через команды выше.",
                "Если возникает ошибка, бот ответит текстом причины. Сообщения длиной более 2000 символов разбиваются на несколько частей автоматически.",
            ]
        ),
    ]
